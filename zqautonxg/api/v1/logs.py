...
# Optimized: Using deque for O(1) appends and automatic size management
logs_history: collections.deque[dict[str, Any]] = collections.deque(maxlen=1000)
...
async def broadcast_log(log_entry: LogEntry) -> None:
    ...
    # Optimized: Use asyncio.gather for parallel broadcasting
    ...
    disconnected = []
    for connection, result in zip(connections_snapshot, results, strict=True):
...
async def logs_websocket(websocket: WebSocket) -> None:
    ...
    # Send recent history
    try:
        # Optimized: Convert deque to list for slicing
...
async def query_logs(
    ...
    if isinstance(filtered_logs, collections.deque):
